            ...
            <?php
                ...
                if (!isset($vars['entity'])) {
                    ...
                    if (get_plugin_setting('allow_groups', 'edifice') == 'yes') {
                        echo elgg_view('edifice/forms/assign', array('current_category' => $current_category, 'groupedit' => 'groupedit'));
                        echo '<br>';
                    }
                } else {
                
		// les 2 lignes en dessous devraient etre viree D2T
		    //echo elgg_echo('edifice:group:current') . get_entity(get_item_categories($vars['entity']->guid))->title . '<br>';
                    //echo elgg_echo('edifice:group:assigned');


		// Ajout de D2T pour changer les groupes meme ancien
		$current_category = NULL;
		?>
		<!-- <div class="title"><h3><?php echo elgg_echo('edifice:group:current') . get_entity(get_item_categories($vars['entity']->guid))->title . '<br>' ?></h3></div> -->
		 <?php
		   echo elgg_echo('edifice:group:current') . get_entity(get_item_categories($vars['entity']->guid))->title . '<br>';
